...
class GiveawayManager {
    ...
    async _check(interval) {
        setInterval(async () => {
            const giveaways = await giveawayModel_1.giveawayModel.find({ ended: false });
    ...
    async enter(messageId, userId) {
        const giveaway = await giveawayModel_1.giveawayModel.findOne({
            messageId,
        });
        if (!giveaway)
    ...
    async end(messageId) {
        ...
        await giveawayModel_1.giveawayModel.updateOne({ messageId }, { ended: true, end: Date.now(), winners });
        const embed = new discord_js_1.MessageEmbed()
            .setTitle(`<:bluedot:842408037502550106> **${giveaway.prize}** <:bluedot:842408037502550106>`)
    ...
    async reroll(messageId, interaction) {
        ...
        const row2 = new discord_js_1.MessageActionRow().addComponents(new discord_js_1.MessageButton().setStyle('SUCCESS').setLabel('Confirm').setCustomId(`reroll-confirm-${messageId}`), new discord_js_1.MessageButton().setStyle('DANGER').setLabel('Cancel').setCustomId(`reroll-cancel-${messageId}`));
        const embed = new discord_js_1.MessageEmbed().setDescription('Select/Unselect the members to reroll below.').setColor('GREEN');
        for (const w of giveaway.winners) {
            const user = await this._client.users.fetch(w);
            row.addComponents(new discord_js_1.MessageButton().setStyle('PRIMARY').setLabel(user.tag).setCustomId(`reroll-select-${w}`));
        }
        await interaction.reply({
            embeds: [embed],
            components: [row, row2],
        ...
        row.components.forEach((c) => {
            if (!(c instanceof discord_js_1.MessageButton))
                return;
        ...
        return new Promise((resolve, reject) => {
            ...
            collector?.on('collect', async (i) => {
                await i.deferUpdate();
                const rerolls = row.components.filter((c) => c instanceof discord_js_1.MessageButton && c.style === 'PRIMARY').map((c) => c.customId?.replace('reroll-select-', ''));
                ...
                const gEmbed = new discord_js_1.MessageEmbed()
                    .setTitle(`<:bluedot:842408037502550106> **${giveaway.prize}** <:bluedot:842408037502550106>`)
                    .setDescription(`**Hosted by:** <@${giveaway.host}>\n${giveaway.sponsor ? `**Sponsored by:** <@${giveaway.sponsor}>\n` : ''}**Ended:** <t:${Math.floor(giveaway.end / 1000)}:R>\n${giveaway.requirement ? `**Requirement:** ${giveaway.requirement}\n` : ''}\n **Winners:** ${winners.map((w) => `<@${w}>`).join(', ')}`)
                    .setFooter(`Message ID: ${message.id} | Winners: ${giveaway.winnerCount}`)
                    .setColor('RED');
                await message.edit({ embeds: [gEmbed] });
                const gRow = new discord_js_1.MessageActionRow().addComponents(new discord_js_1.MessageButton().setStyle('DANGER').setLabel('Control').setCustomId(`control-giveaway-${giveaway.messageId}`), new discord_js_1.MessageButton().setStyle('LINK').setURL(message.url).setLabel(`${giveaway.entries.length + winners.length} entries`));
                await channel.send({
                    content: `Congratulations ${mentions.join(', ')}, you have won **${giveaway.prize}**!`,
                    components: [gRow],
                });
                await giveawayModel_1.giveawayModel.updateOne({ messageId }, { winners });
