    ...
    async run(interaction, options, client) {
        ...
        const pages = [
            {
                ...
                Subtotal: logs.filter((r) => ['Warn', 'Mute', 'Unmute', 'Ban', 'Unban'].includes(r.caseType)),
            },
            {
                Blacklists: logs.filter((r) => r.caseType === 'Blacklist'),
                Unblacklists: logs.filter((r) => r.caseType === 'Unblacklist'),
                'Moderated Nicknames': logs.filter((r) => r.caseType === 'Moderated Nickname'),
        ...
        pages.forEach((page, i) => {
            ...
            for (v in page) {
                embeds[i].addFields({
                    name: `${v}\n(last 7 days)`,
                    value: page[v].filter((l) => Date.now() - l.timestamp < 7 * day).length.toString(),
                    inline: true,
                }, {
                    name: `${v}\n(last 30 days)`,
                    value: page[v].filter((l) => Date.now() - l.timestamp < 30 * day).length.toString(),
                    inline: true,
                }, {
                    name: `${v}\n(all time)`,
                    value: page[v].length.toString(),
                    inline: true,
                });
            }
        });
